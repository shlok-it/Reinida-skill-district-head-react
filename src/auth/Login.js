...
const Login = () => {
	...
	const check_session = () => {
		const authenticated = secureLocalStorage.getItem("district_head_authenticated");
		if (authenticated) {
			const get_profile = secureLocalStorage.getItem("district_head_access_token");
			if (get_profile) {
				setLoading(true);
					...
					.then(
						(resolve) => {
							if (resolve.status === true) {
								window.location = "/district-head/dashboard";
							}
							else {
								setLoading(false);
							}
			...
			.then(
				(resolve) => {
					if (resolve.status === true) {
						...
						secureLocalStorage.setItem("district_head_authenticated", true);
						secureLocalStorage.setItem("district_head_access_token", resolve.data['accessToken']);
						window.location = "/district-head/dashboard";
					}
					else {

...
const Career = () => {
  ...
  const get_emp_list = (pagenum = page, size = perPage,col=colName,sort=sortBy) => {
    setLoading(true);
    call_secure_get_api(
      `career/list?page=${pagenum}&per_page=${size}&colName=${col}&sortBy=${sort}&search_key=${search_key}&delay=1`
    ).then(
      (resolve) => {
    ...
    <React.Fragment>
      <div className="card radius-10">
        ...
        <div className="d-lg-flex align-items-center mb-4 gap-3">
            ...
            <div><button type="button" onClick={(e)=>searchFun(e)} className="btn btn-success">Search</button></div>
          </div>
          {display}
          <DataTableShow 
            // title="Career Form"
